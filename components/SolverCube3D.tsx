 ...
 *
 * This reuses the homepage cube's rounded black cubies, raised stickers,
 * lighting, camera, and drag controls. At step zero it renders the scrambled
 * state. Moving forward or backward rotates the affected physical layer and
 * then rebuilds sticker colors from the verified cube engine state.
 */
...
type MoveSpec = { axis: "x"|"y"|"z"; layer: -1|0|1; angle: number };
...
function Sticker({color,position,rotation=[0,0,0]}:{color:string;position:Vec3;rotation?:Vec3}){
  return <RoundedBox args={[.76,.76,.045]} radius={.055} smoothness={3} position={position} rotation={rotation}>
    <meshStandardMaterial color={color} roughness={.3} metalness={.02} emissive={color} emissiveIntensity={.035}/>
  </RoundedBox>;
}
...
function Scene({scramble,solution,step,onAnimating}:{scramble:string;solution:string[];step:number;onAnimating:(v:boolean)=>void}){
  ...
  const animation=useRef<{axis:"x"|"y"|"z";from:number;to:number;start:number;next:string}|null>(null);

  useEffect(()=>{setFacelets(initial);shownStep.current=0;animation.current=null;onAnimating(false);},[initial,onAnimating]);
  useEffect(()=>{
    ...
    animation.current={axis:target.axis,from:0,to:target.angle,start:performance.now(),next};
    shownStep.current=nextStep;onAnimating(true);
  },[step,solution,scramble,onAnimating]);

  useFrame(()=>{
    const a=animation.current,g=layerRef.current;if(!a||!g)return;
    const t=Math.min(1,(performance.now()-a.start)/420);const eased=1-Math.pow(1-t,3);
    g.rotation[a.axis]=a.from+(a.to-a.from)*eased;
    if(t===1){g.rotation.set(0,0,0);setFacelets(a.next);animation.current=null;onAnimating(false);}
  });

  const active=step===0||!solution.length?null:spec(solution[Math.max(0,step-1)]);
  const cells=[] as {x:number;y:number;z:number}[];for(let x=-1;x<=1;x++)for(let y=-1;y<=1;y++)for(let z=-1;z<=1;z++)cells.push({x,y,z});
  const inLayer=(c:{x:number;y:number;z:number})=>active?c[active.axis]===active.layer:false;
  return <>
    <hemisphereLight args={["#ffffff","#18345f",1.65]}/><directionalLight position={[5,8,7]} intensity={2.15}/><directionalLight position={[-5,3,4]} intensity={1.15} color="#9fd8ff"/>
...
export default function SolverCube3D({scramble,solution,step,onAnimating}:{scramble:string;solution:string[];step:number;onAnimating:(v:boolean)=>void}){
  return <Canvas camera={{position:[4.4,3.5,5.1],fov:36}} dpr={[1,1.75]} gl={{antialias:true,alpha:true}} style={{background:"transparent"}}>
