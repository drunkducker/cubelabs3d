...
/** Return the translation component stored in a Matrix4. */
...
/**
 * Force a completed cube transform onto the exact Rubik's Cube lattice.
 * A legal cubie transform contains only signed basis vectors and integer
 * translations, so values such as 0.9999999998 must become exactly 1.
 */
function snapCubeMatrix(matrix: THREE.Matrix4) {
  const snapped = matrix.clone();
  const elements = snapped.elements;

  // The upper-left 3x3 block stores orientation. Legal quarter turns produce
  // only -1, 0, and 1 in these entries.
...
function Scene({
  ...
  /** Keep an exact registry of the 27 permanent cubie groups. */
  ...
  useEffect(() => {
    ...
    // Read the layer from each cubie's exact matrix translation. We clone every
    // starting matrix so the animation never compounds frame-over-frame error.
    ...
    // A face layer must always contain exactly nine cubies. Refusing to animate
    // an invalid selection is safer than visually tearing the cube apart.
  ...
  useFrame(() => {
    ...
    current.cubies.forEach(({ group, matrix }) => {
      // Premultiplication rotates the cubie and its center around the cube origin.
...
export default function SolverCube3D({
  scramble,
  solution,
  step,
  onAnimating,
  ...
  // A new scramble remounts the scene once, building a fresh exact physical cube.
