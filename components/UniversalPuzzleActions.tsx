...
export default function UniversalPuzzleActions() {
  ...
  }, [puzzleType, queryScramble, refresh, toast]);
  ...
  const applyScramble = (value: string) => {
    ...
    toast("Scramble selected and copied. The puzzle will load it automatically where supported.");
  };
      ...
      {open ? (
        <section className="glass max-h-[78dvh] overflow-y-auto rounded-[20px] border border-white/15 p-4 shadow-2xl">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[.16em] text-[var(--green)]">{puzzleType} memory + friend play</p>
              <h2 className="mt-1 text-lg font-black text-white">Save it. Replay it. Send it.</h2>
            </div>
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            <button onClick={() => applyScramble(scramble)} disabled={busy || !scramble.trim()} className="glass min-h-11 rounded-xl font-black disabled:opacity-40">Load / copy</button>
            <button onClick={saveMemory} disabled={busy || !scramble.trim()} className="cta-green min-h-11 rounded-xl font-black disabled:opacity-40">Save memory</button>
          </div>
        ...
        <button onClick={() => setOpen(true)} className="cta-purple ml-auto flex min-h-12 items-center gap-2 rounded-full px-5 font-black shadow-2xl">
