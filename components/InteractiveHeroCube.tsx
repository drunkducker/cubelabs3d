...
type Gesture = { pointerId: number; startX: number; startY: number; group: THREE.Group; normal: THREE.Vector3; moved: boolean; target: Element | null };
...
function Sticker({ color, position, rotation: turn=[0,0,0], normal, groupRef, onMove, controls }: { color:string; position:[number,number,number]; rotation?:[number,number,number]; normal:[number,number,number]; groupRef:React.RefObject<THREE.Group|null>; onMove:(move:string)=>void; controls:React.RefObject<OrbitControlsImpl|null> }) {
  ...
  const releaseGesture=()=>{const g=gesture.current;if(g?.target&&g.target.hasPointerCapture?.(g.pointerId))g.target.releasePointerCapture?.(g.pointerId);gesture.current=null;if(controls.current)controls.current.enabled=true;};
  const down=(event:ThreeEvent<PointerEvent>)=>{const group=groupRef.current;if(!group)return;event.stopPropagation();if(controls.current)controls.current.enabled=false;const target=event.target as Element;target.setPointerCapture?.(event.pointerId);const matrix3=new THREE.Matrix3().setFromMatrix4(group.matrix);gesture.current={pointerId:event.pointerId,startX:event.clientX,startY:event.clientY,group,normal:snapAxis(new THREE.Vector3(...normal).applyMatrix3(matrix3)),moved:false,target};};
  const move=(event:ThreeEvent<PointerEvent>)=>{const g=gesture.current;if(!g||g.pointerId!==event.pointerId||g.moved)return;const delta=new THREE.Vector2(event.clientX-g.startX,event.clientY-g.startY);if(delta.length()<13)return;event.stopPropagation();g.moved=true;const candidates=[axisVector("x"),axisVector("y"),axisVector("z")].filter(v=>Math.abs(v.dot(g.normal))<.5);const origin=g.group.getWorldPosition(new THREE.Vector3()),pointer=delta.normalize();let best=candidates[0],bestDot=-1,sign=1;for(const c of candidates){const dot=screenDirection(origin,c).dot(pointer);if(Math.abs(dot)>bestDot){bestDot=Math.abs(dot);best=c;sign=dot>=0?1:-1;}}const tangent=best.clone().multiplyScalar(sign),signed=snapAxis(new THREE.Vector3().crossVectors(g.normal,tangent)),axis=axisName(signed),dir=Math.sign(signed[axis])||1,layer=Math.round(coordinate(g.group.matrix,axis));releaseGesture();onMove(notation(axis,layer,dir));};
  const up=(event:ThreeEvent<PointerEvent>)=>{if(gesture.current?.pointerId!==event.pointerId)return;releaseGesture();};
  useEffect(()=>()=>releaseGesture(),[]);
  return <RoundedBox args={[.76,.76,.045]} radius={.055} smoothness={3} position={position} rotation={turn} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={up} onPointerOut={up}><meshStandardMaterial color={color} roughness={.3} metalness={.02} emissive={color} emissiveIntensity={.035}/></RoundedBox>;
}
