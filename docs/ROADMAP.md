...
atomic checklist. Items are checked only when repository evidence and required
documentation support completion.
...
5. Keep `CURRENT_STATUS.md`, this roadmap, the canonical topic reference, and
   `HISTORY.md` aligned.
...
- [ ] Complete legal identity, final policies, cookie consent, age screening,
      moderation, and accessibility launch blockers in the atomic checklist
      below.
...
- [~] Cookie-cutter kit: engine-grouped puzzle registry (`lib/puzzles.mjs`, solve/play/learn features + waivers + experiment hatch) and site-module registry (`lib/modules.mjs`, affiliate carousel/ad/banner/YouTube/footer), with `scripts/check-puzzles.mjs` + `scripts/check-modules.mjs` conformance checkers wired into CI (`npm run cc:check`), printing ✅/⚠️/❌ matrices (ADR 0008). Branch-only on `claude/kilominx-solver-3d-page-wbyyay`
- [ ] Build the YouTube video carousel module (declared but waived in `lib/modules.mjs`)
...
The numbered atomic checklist is now part of the roadmap. Keep its stable task
IDs when updating status so earlier logs remain searchable.

---

## Atomic master checklist

> Consolidated from `docs/MASTER-CHECKLIST.md` on 2026-07-27. The source path remains
> recorded here so repository history can recover the exact earlier file.

<!-- BEGIN CONSOLIDATED SOURCE: docs/MASTER-CHECKLIST.md -->
...
- Record the commit, PR, migration, test, screenshot, or production verification
  in `HISTORY.md`.
...
<!-- END CONSOLIDATED SOURCE: docs/MASTER-CHECKLIST.md -->
