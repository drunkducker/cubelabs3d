...
export async function signIn(formData: FormData) {
   ...
   * unavailable (migration not applied) so real users are never locked out by
   * an infrastructure gap; activates automatically once the 20260726 migration
   * is in place.
   */
  const ip = clientIp();
...
export async function requestPasswordReset(formData: FormData) {
  ...
  // Response stays uniform to avoid account enumeration.
