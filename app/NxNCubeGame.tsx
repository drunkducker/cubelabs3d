...
const COLORS = {
  R: "#e52b3d", L: "#ff7a00", U: "#f5f1e8", D: "#ffd500",
  F: "#00a85a", B: "#1557d5", I: "#111319",
};
...
export default function NxNCubeGame({ size = 10 }: { size?: number }) {
  ...
  useEffect(() => {
    ...
    const homePosition = new THREE.Vector3(distance * 0.62, distance * 0.54, distance);
    ...
    const clearHighlight = () => {
      highlighted.forEach((cubie) => {
        const materials = Array.isArray(cubie.mesh.material) ? cubie.mesh.material : [cubie.mesh.material];
    ...
    const runNext = () => {
      ...
      const selected = cubies.filter((cubie) => Math.abs(cubie.grid[move.axis] - move.layer) < 0.01);
      ...
      const animate = (now: number) => {
        ...
        const rotation = new THREE.Matrix4().makeRotationAxis(axisVector(move.axis), move.direction * Math.PI * 0.5);
        selected.forEach((cubie) => {
          root.attach(cubie.mesh);
          cubie.mesh.position.set(snap(cubie.mesh.position.x, size), snap(cubie.mesh.position.y, size), snap(cubie.mesh.position.z, size));
          cubie.grid.applyMatrix4(rotation).set(snap(cubie.grid.x, size), snap(cubie.grid.y, size), snap(cubie.grid.z, size));
        });
        root.remove(pivot);
    ...
    const scramble = () => {
      ...
      for (let index = 0; index < 16; index += 1) {
        ...
        queue.push({ ...base, direction: Math.random() > 0.5 ? base.direction : (base.direction * -1) as Direction, record: true });
      }
    ...
    const undo = () => {
      ...
      queue.push({
        ...
        direction: (previous.direction * -1) as Direction,
        label: `Undo ${previous.label}`,
        record: false,
      });
      setMoves(history.length);
      runNext();
    ...
    actionsRef.current = { turn, scramble, undo, resetCube, resetView };
    ...
    const onPointerDown = (event: PointerEvent) => {
      ...
      const hit = raycaster.intersectObjects(cubies.map((cubie) => cubie.mesh), false)[0];
    ...
    const resolveGesture = (start: PointerStart, dx: number, dy: number) => {
      ...
      return {
        axis: rotationAxis,
        layer: start.cubie.grid[rotationAxis],
        direction: (Math.sign(rotationVector[rotationAxis]) || 1) as Direction,
      };
    };
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      highlightLayer(gesture.axis, gesture.layer);
      setStatus(`Selected ${moveLabel(gesture.axis, gesture.layer, edge, 1)} layer`);
    };
    ...
    const finishPointer = (event: PointerEvent) => {
      ...
      if (distanceDragged >= 28 && !active) {
        const gesture = resolveGesture(start, dx, dy);
        turn({
          ...gesture,
          label: moveLabel(gesture.axis, gesture.layer, edge, gesture.direction),
        });
      } else {
        clearHighlight();
    ...
    <main className="app-shell relative min-h-dvh w-full max-w-[460px] overflow-hidden px-5 pb-[calc(28px+env(safe-area-inset-bottom))] pt-[22px]">
      ...
      <div className="relative z-[1]">
        ...
        <section className="mt-5">
          ...
          <p className="mt-3 text-[15px] leading-6 text-[var(--muted)]">Swipe any visible section to turn that exact layer. Rotate, zoom, and pan to reach every edge and corner.</p>
        </section>
        ...
        <div className="mt-3 grid grid-cols-3 gap-2">
          {Object.entries(faceMoves).map(([label, move]) => (
            <button key={label} disabled={busy} onClick={() => actionsRef.current?.turn(move)} className="glass min-h-12 rounded-xl font-extrabold disabled:opacity-40">{label}</button>
          ))}
        </div>
        ...
        <section className="glass mt-4 rounded-[18px] p-4 text-sm leading-6 text-[var(--muted)]">
          <p><strong className="text-[var(--text)]">On a sticker:</strong> hold your swipe long enough to highlight the exact layer, then release to turn it.</p>
