...
const FACE_COLORS = ["#f7f7f2", "#ffd500", "#00a85a", "#1261d6", "#ef3340", "#ff7a00"];
...
function roundedShape(points: THREE.Vector2[], radius: number) {
  ...
  points.forEach((current, index) => {
    ...
    const previousDistance = current.distanceTo(previous);
    const nextDistance = current.distanceTo(next);
    const cornerRadius = Math.min(radius, previousDistance * 0.24, nextDistance * 0.24);
    starts.push(current.clone().lerp(previous, cornerRadius / previousDistance));
    ends.push(current.clone().lerp(next, cornerRadius / nextDistance));
  });
...
function inset2d(points: THREE.Vector2[], scale: number) {
...
function premiumPanelGeometry(
  ...
  const points2d = inset2d(
    points.map(point => new THREE.Vector2(point.dot(face.u), point.dot(face.v))),
    options.inset,
  );
  const geometry = new THREE.ExtrudeGeometry(roundedShape(points2d, options.radius), {
    depth: options.depth,
    steps: 1,
    curveSegments: 8,
    bevelEnabled: true,
...
export default function SkewbGamePremium() {
  ...
  useEffect(() => {
    ...
    const distance = 5.35;
    camera.position.set(distance * 0.84, distance * 0.7, distance);
    ...
    scene.add(new THREE.HemisphereLight("#ffffff", "#15223c", 2.05));
    const key = new THREE.DirectionalLight("#ffffff", 3.2);
    key.position.set(7, 11, 9);
    key.castShadow = true;
    scene.add(key);
    const fill = new THREE.DirectionalLight("#8cc8ff", 1.45);
    fill.position.set(-8, 2, 6);
    scene.add(fill);
    const rim = new THREE.DirectionalLight("#5f70ff", 1.8);
    ...
    FACE_DATA.forEach((face, faceIndex) => {
      ...
      polygons.forEach(poly => {
        ...
        if (poly.kind === "center" && faceIndex === 0) {
    ...
    const glowPiece = (group: THREE.Group, intensity: number) => {
      group.traverse(child => {
        ...
        material.emissiveIntensity = intensity || 0.018;
      });
    };
    ...
    const highlightMove = (gesture: SwipeGesture) => {
      ...
      highlighted.forEach(group => glowPiece(group, 0.22));
    };
    ...
    const runNext = () => {
      ...
      const duration = move.fast ? QUICK_TURN_DURATION_MS : Math.max(MIN_SETTLE_DURATION_MS, TURN_DURATION_MS * remaining);
      clearHighlight();
    ...
    const turn = (
      ...
      enqueue({ axis, direction, ...(layer === -1 ? { layer } : {}), phase: "play", startAngle });
    };
    ...
    const resolveGesture = (start: PointerStart, dx: number, dy: number): SwipeGesture => {
      ...
      for (const candidate of candidates) {
        const axisWorld = new THREE.Vector3(...pivotVector(candidate)).normalize().transformDirection(root.matrixWorld);
  ...
  return <main className="app-shell relative min-h-dvh w-full max-w-[460px] overflow-hidden px-5 pb-[calc(28px+env(safe-area-inset-bottom))] pt-[22px]">
    ...
    <div className="relative z-[1]">
      ...
      <section className="mt-5">
        ...
        <p className="mt-3 text-[15px] leading-6 text-[var(--muted)]">Swipe a premium, beveled Skewb model through true 120° turns, then solve any state.</p>
      </section>
      <section className="cube-card mt-4 overflow-hidden rounded-[22px]">
        ...
        <div ref={mountRef} role="application" aria-label="Interactive premium Skewb puzzle" data-testid="skewb-canvas" className="h-[430px] w-full touch-none sm:h-[480px]" />
