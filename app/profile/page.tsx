...
type Achievement = {
  ...
  unlocked_at: string;
  achievements?: { name: string; icon: string | null; points?: number | null } | null;
};
type Challenge = {
  id: string;
  puzzle_type: string;
...
const previewCubes: CollectionItem[] = [
...
export default async function ProfilePage() {
  ...
  const [profiles, solves, statsRows, collection, achievements, challenges, friendships] = await Promise.all([
    safeRequest(
      supabaseRequest<Profile[]>(
        `/rest/v1/profiles?id=eq.${user.id}&select=username,display_name,cube_tag,public_slug,avatar_url,title,bio,favorite_puzzle,country_code,region,show_location,created_at`,
        {},
        token,
      ),
      [],
    ...
    safeRequest(
      supabaseRequest<Solve[]>(
        `/rest/v1/solve_results?user_id=eq.${user.id}&select=id,puzzle_type,scramble,solve_time_ms,move_count,solved,is_dnf,leaderboard_eligible,created_at&order=created_at.desc&limit=8`,
        {},
        token,
      ),
      [],
    ...
    safeRequest(
      supabaseRequest<Stats[]>(
        `/rest/v1/user_stats?user_id=eq.${user.id}&select=total_solves,solved_count,current_streak,longest_streak,best_times,averages`,
        {},
        token,
      ),
      [],
    ...
    safeRequest(
      supabaseRequest<CollectionItem[]>(
        `/rest/v1/cube_collection?user_id=eq.${user.id}&select=id,brand,model,nickname,puzzle_type,image_url,is_favorite&order=is_favorite.desc,created_at.desc&limit=6`,
        {},
        token,
      ),
      [],
    ...
    safeRequest(
      supabaseRequest<Achievement[]>(
        `/rest/v1/user_achievements?user_id=eq.${user.id}&select=achievement_id,unlocked_at,achievements(name,icon,points)&order=unlocked_at.desc&limit=6`,
        {},
        token,
      ),
      [],
    ...
    safeRequest(
      supabaseRequest<Challenge[]>(
        `/rest/v1/challenges?or=(creator_id.eq.${user.id},sender_id.eq.${user.id},recipient_id.eq.${user.id})&select=id,puzzle_type,status,scramble,message,sender_time_ms,creator_time_ms,created_at&order=created_at.desc&limit=4`,
        {},
        token,
      ),
      [],
    ...
    safeRequest(
      supabaseRequest<Friendship[]>(
        `/rest/v1/friendships?or=(requester_id.eq.${user.id},addressee_id.eq.${user.id})&status=eq.accepted&select=id,status`,
        {},
        token,
      ),
      [],
      ...
      <div className="relative z-[1]">
        ...
        <section aria-label="Profile stats" className="mt-4 overflow-x-auto pb-1">
          <div className="grid min-w-[670px] grid-cols-5 gap-1">
            ...
            <StatCard icon={TrendIcon} value={publicRank} label="Global Rank" helper="Top 3%" tone="green" />
          </div>
        </section>
  ...
  return (
    <article className="rounded-[8px] border border-[var(--border)] bg-white/[.04] p-4">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 pl-[64px]">
        <button type="button" className="min-h-10 rounded-[8px] border border-[var(--border-2)] bg-black/20 text-[14px] font-semibold text-slate-300">
          Decline
        </button>
