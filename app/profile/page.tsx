...
type Solve = { id: string; puzzle_type: string; solve_time_ms: number | null; move_count: number | null; solved: boolean; is_dnf: boolean; created_at: string };
...
export default async function ProfilePage() {
  ...
  const profiles = await supabaseRequest<Profile[]>(`/rest/v1/profiles?id=eq.${user.id}&select=username,display_name,avatar_url`, {}, token);
  const solves = await supabaseRequest<Solve[]>(`/rest/v1/solve_results?user_id=eq.${user.id}&select=id,puzzle_type,solve_time_ms,move_count,solved,is_dnf,created_at&order=created_at.desc&limit=25`, {}, token);
