...
export async function getSecuritySummary(): Promise<{ checks: SecurityCheck[]; events: Array<{ id: string; event_type: string; severity: string; created_at: string; resolved: boolean }> }> {
  ...
  checks.push({
    ...
    status: "manual",
    detail: "Sign-in lockout, password-reset throttling, admin-action throttling, media upload, checkout, MFA verify, and ad tracking all call check_rate_limit. Fails open until 20260726_rate_limiting.sql is applied; then it silently activates.",
  });

  // Security response headers ship in next.config.mjs; the response cannot be
  // introspected here, so this is a labelled-manual check pointing at config.
  checks.push({
    id: "security_headers",
    label: "Security response headers configured",
    status: "manual",
    detail: "Content-Security-Policy, X-Frame-Options DENY, nosniff, Referrer-Policy, Permissions-Policy, and COOP are set in next.config.mjs. Verify with curl -I on production.",
  });
